...
4. Notion: Zielworkspace ist "Workspace von SWOT" (existiert, Richard ist Admin). Der Claude-Notion-Connector haengt am Jolly-Workspace und sieht den SWOT-Workspace nicht; alle Schreibzugriffe laufen ueber die REST-API mit der internen Integration "SWOT Content Engine" (angelegt 2026-09-22, Rechte Read/Update/Insert content, Token liegt lokal als `NOTION_TOKEN_SWOT` in der Content-Engine-.env und wird `NOTION_TOKEN` des neuen Repos; Content access auf den Teamspace "Workspace von SWOT HQ", `users/me` liefert Workspace "Workspace von SWOT"). Zielseite fuer beide DBs ist "Linkedin Content Engine" (`3e36b49c-cad0-8096-b851-e8ac6879da98`) in diesem Teamspace. Richard dupliziert Redaktionsplan-DB und Topic-Ideas-DB per Notion-UI unter diese Seite (Inhalte und Views kommen mit); die Integration sieht sie ueber den Teamspace automatisch. Danach schreibt ein Skript die Bild-Properties der Kopie auf die Supabase-URLs um (einmalig, Trockenlauf zuerst). Readback: Zeilenzahl beider DBs Quelle gleich Ziel, null Bild-URLs mit `githubusercontent`.
