...
3. `tools/kieai_image.py`: Upload nach Supabase Storage statt GitHub. Bucket `post-images`, public, Pfad `<yyyy-mm>/<slug>.png`, URL `SUPABASE_URL/storage/v1/object/public/post-images/<pfad>`. `GITHUB_TOKEN`, `GITHUB_REPO`, `GITHUB_IMAGES_BRANCH` und der catbox-Fallback entfallen. Logo-Default `swot_logo.png`.
...
4. Notion: SWOT richtet einen eigenen Workspace ein oder benennt einen bestehenden. Redaktionsplan-DB und Topic-Ideas-DB dorthin duplizieren (Notion "Duplicate to", Inhalte und Views kommen mit). Bild-Properties in der Kopie auf die Supabase-URLs umschreiben (Skript, einmalig, Trockenlauf zuerst). SWOT legt eine interne Integration an und teilt beide DBs damit. Readback: Zeilenzahl beider DBs Quelle gleich Ziel, null Bild-URLs mit `githubusercontent`.
